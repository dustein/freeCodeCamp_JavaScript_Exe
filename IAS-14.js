function mmc(arr) {
    ...
    // maior valor possivel para o MMC - minimo multiplicador comum
    let majorante = 1
    for (let i = min; i <= max; i++) {
      majorante *= i
    }
    // testar todos os multiplos de 'max'
    for (let multiplo = max; multiplo <= majorante; multiplo += max) {
      // checar se todos os valores no range sao divisores de 'multiplo'
      let divisorCount = 0
      for (let i = min; i <= max; i++) {
        // Count divisors
        if (multiplo % i === 0) {
